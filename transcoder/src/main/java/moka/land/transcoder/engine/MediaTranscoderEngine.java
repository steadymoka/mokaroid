...
import moka.land.transcoder.format.MediaFormatStrategy;
import moka.land.transcoder.utils.ISO6709LocationParser;
import moka.land.transcoder.utils.MediaExtractorUtils;
...
public class MediaTranscoderEngine {
    ...
    private void setupTrackTranscoders(MediaFormatStrategy formatStrategy) {
        ...
        if (trackResult.mVideoTrackFormat != null && trackResult.mAudioTrackFormat != null) {
            setupTrackTranscoders(formatStrategy, trackResult);
        } else {
            setupTrackTranscodersOnlyVideo(formatStrategy, trackResult);
        }
    }

    private void setupTrackTranscoders(MediaFormatStrategy formatStrategy, MediaExtractorUtils.TrackResult trackResult) {
        ...
        if (-1 != trackResult.mAudioTrackIndex) {
            mExtractor.selectTrack(trackResult.mAudioTrackIndex);
        }
    }

    private void setupTrackTranscodersOnlyVideo(MediaFormatStrategy formatStrategy, MediaExtractorUtils.TrackResult trackResult) {
        MediaFormat videoOutputFormat = formatStrategy.createVideoOutputFormat(trackResult.mVideoTrackFormat);

        if (videoOutputFormat == null) {
            throw new InvalidOutputFormatException("MediaFormatStrategy returned pass-through for both video and audio. No transcoding is necessary.");
        }
        QueuedMuxerOnlyVideo queuedMuxer = new QueuedMuxerOnlyVideo(mMuxer, new QueuedMuxerOnlyVideo.Listener() {
            @Override
            public void onDetermineOutputFormat() {
                MediaFormatValidator.validateVideoOutputFormat(mVideoTrackTranscoder.getDeterminedFormat());
            }
        ...
        mVideoTrackTranscoder = new VideoTrackTranscoderOnlyVideo(mExtractor, trackResult.mVideoTrackIndex, videoOutputFormat, queuedMuxer);
        mVideoTrackTranscoder.setup();
    ...
    private void runPipelines() throws InterruptedException {
        if (null == mAudioTrackTranscoder) {
            runPipelinesOnlyVideo();
            return;
        }
    ...
    private void runPipelinesOnlyVideo() throws InterruptedException {
        long loopCount = 0;
        if (mDurationUs <= 0) {
            double progress = PROGRESS_UNKNOWN;
            mProgress = progress;
        ...
        while (!(mVideoTrackTranscoder.isFinished())) {
            ...
            loopCount++;
            if (mDurationUs > 0 && loopCount % PROGRESS_INTERVAL_STEPS == 0) {
                double progress = mVideoTrackTranscoder.isFinished() ? 1.0 : Math.min(1.0, (double) mVideoTrackTranscoder.getWrittenPresentationTimeUs() / mDurationUs);
                mProgress = progress;
